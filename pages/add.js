...
export default function AddAct() {
  ...
  try {
    ...
    if (form.pdf) {
      ...
      const { data } = supabase.storage.from('acts-files').getPublicUrl(pdfFileName);
      pdfUrl = data?.publicUrl ?? null; // якщо чогось нема, ставимо null
    }
    ...
    if (form.photo) {
      ...
      const { data } = supabase.storage.from('acts-files').getPublicUrl(photoFileName);
      photoUrl = data?.publicUrl ?? null; // якщо чогось нема, ставимо null
    }

    // Вставка рядка в таблицю acts
    const { data: newRow, error } = await supabase.from('acts').insert([{
      date: form.date,
      amount: form.amount,
      ...
      pdf_url: pdfUrl,
      photo_url: photoUrl
    }]).select().single(); // .select().single() повертає вставлений рядок
